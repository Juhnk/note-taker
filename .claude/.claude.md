...
- Blocks commit if:
  ...
  - Tests fail
  - Coverage < 70%
- Cannot be bypassed (ensures quality)
...
8. **Commit with conventional commits**:
   ...
   ```
   Note: Pre-commit hook will run automatically and block commit if:
   - SwiftLint fails
   - Build fails
   - Tests fail
...
- [ ] Test coverage ≥ 70% (target varies by component)
...
4. **Write tests FIRST** (TDD approach, 70%+ coverage required)
...
9. Commit with conventional messages (pre-commit hook will enforce quality)
